...
class LineByLineTextDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, file_path, gold_path):
        ...
        cache_fn = f'{file_path}.bert.cache' if gold_path is None else f'{file_path}.bert.gold.cache'
        if args.cache_data and os.path.isfile(cache_fn) and not args.overwrite_cache:
            ...
            for line_id, line in tqdm(enumerate(lines), desc='Loading data', total=len(lines)):
                if len(line) > 0 and not line.isspace() and len(line.split(' ||| ')) == 2:
                    ...
                    ids_src, ids_tgt = tokenizer.prepare_for_model(list(itertools.chain(*wid_src)), return_tensors='pt', max_length=args.max_len, truncation=True)['input_ids'], tokenizer.prepare_for_model(list(itertools.chain(*wid_tgt)), return_tensors='pt', max_length=args.max_len, truncation=True)['input_ids']
...
def train(args, train_dataset, model: PreTrainedModel, model_stage1: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    """ Train the model """
    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)
    def collate(examples):
        model.eval()
        model_stage1.eval()
        examples_src, examples_tgt, psi_examples_src, psi_examples_tgt, psi_labels = [], [], [], [], []
        src_len = tgt_len = 0
        bpe2word_map_src, bpe2word_map_tgt = [], []
        word_aligns = []
        for example in examples:
            end_id = example[0].view(-1)
        ...
        if args.n_gpu > 1 or args.local_rank != -1:
            guides = model_stage1.get_aligned_subword(examples_src, examples_tgt, bpe2word_map_src, bpe2word_map_tgt,
            ...
            guides = model_stage1.get_aligned_subword(examples_src, examples_tgt, bpe2word_map_src, bpe2word_map_tgt, args.device,
                                            src_len, tgt_len, extraction=args.extraction, align_layer=args.align_layer,
    ...
    logger.info("  Total Num Epochs = %d", args.num_train_epochs)
    ...
    loss_inter, loss_intra, loss_consis, loss_kl, loss_cl ,loss_psi= None, None, None,None, None, None
    for epoch in train_iterator:
        epoch_iterator = tqdm(train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])
        logger.info(" Start Epochs = %d", epoch)
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.train_so or args.train_co:
                ...
                loss = model(inputs_src=inputs_src, inputs_tgt=inputs_tgt, attention_mask_src=attention_mask_src,
                             attention_mask_tgt=attention_mask_tgt, align_layer=args.align_layer, guide=guide,
                             extraction=args.extraction, softmax_threshold=args.softmax_threshold,
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    # print("  Step {}. Training loss = {}. Inter loss = {}. Intra loss = {}. Contrast loss ={}".format(str(global_step),
                    #             str((tr_loss - logging_loss) / args.logging_steps),str(loss_inter.mean().item()),str(loss_intra.mean().item()),str(loss_cl.mean().item())))
                    print("  Step {}. Training loss = {}. ".format(str(global_step),
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    logger.info("  Step %s. Saving model to %s", str(global_step),output_dir)
            ...
            if global_step > t_total:
                break
        logger.info(" epoch = %s, average loss = %s", epoch, tr_loss/ global_step)
        print(" epoch = {}, average loss = {}".format(epoch, tr_loss / global_step))
        if global_step > t_total:
            break
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        with torch.no_grad():
            if args.train_tlm:
                ...
                loss = model(inputs_src=inputs_src, inputs_tgt=batch[1].to(args.device),labels_src=labels_src, attention_mask_src=attention_mask_src,attention_mask_tgt=attention_mask_tgt)
                # print(loss)
                eval_loss = post_loss(loss, eval_loss)

                loss = model(inputs_src=inputs_tgt,inputs_tgt=batch[0].to(args.device), labels_src=labels_tgt, attention_mask_src=attention_mask_tgt,attention_mask_tgt=attention_mask_src)
                eval_loss = post_loss(loss, eval_loss)
...
def main():
    ...
    parser.add_argument("--train_cl", action="store_true")
    parser.add_argument("--train_kl", action="store_true")
    # Model configuration
    parser.add_argument("--self_m", type=int, default=0)
    parser.add_argument("--cross_k", type=int, default=0)
    ...
    parser.add_argument(
        "--train_from_scratch", action="store_true", help="Whether to train_from_scratch"
    )
    parser.add_argument(
        "--freeze", action="store_true", help="Whether to freeze domain1"
    )
    parser.add_argument(
        "--model_name_or_path",
        default=None,
    ...
    parser.add_argument(
        "--stage1_model_name_or_path",
        default=None,
        type=str,
        help="The model checkpoint for generating alignment in stage 1."
    )
    parser.add_argument(
    ...
    #there is cross attention layer
# if args.cross_k > 0:
    ...
    if args.block_size <= 0:
        args.block_size = tokenizer.max_model_input_sizes['bert-base-uncased']
        # Our input block size will be the max possible for the model
    else:
        args.block_size = min(args.block_size, tokenizer.max_model_input_sizes['bert-base-uncased'])



    if args.train_from_scratch:
        logger.info("Training new model from scratch")
        config.cross_k = args.cross_k
        config.self_m = args.self_m
    ...
    if args.stage1_model_name_or_path:
        model_stage1 = model_class.from_pretrained(
            args.stage1_model_name_or_path,
            from_tf=bool(".ckpt" in args.stage1_model_name_or_path),
            config=config,
            cache_dir=args.cache_dir,
        ...
        raise ValueError(
            "Set a stage1 model to generate alignment"
        )

    model.to(args.device)
    model_stage1.to(args.device)
    ...
    if args.do_train:
        ...
        global_step, tr_loss = train(args, train_dataset, model, model_stage1, tokenizer)
