...
from transformers import CrossBertForMaskedLM, BertConfig, BertTokenizer, BertForMaskedLM
from transformers import PreTrainedModel, PreTrainedTokenizer
from transformers.train_utils import _sorted_checkpoints, _rotate_checkpoints, WEIGHTS_NAME, AdamW, \
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if steps_trained_in_current_epoch > 0:
                ...
                continue
            tqdm_iterator.update(global_step)
            ...
            if args.train_tlm:
                ...
                attention_mask_src, attention_mask_tgt = (inputs_src != tokenizer.pad_token_id), (
                            inputs_tgt != tokenizer.pad_token_id)
                ...
                attention_mask_src, attention_mask_tgt = attention_mask_src.to(args.device), attention_mask_tgt.to(
                    args.device)

                if random.random() > 0.5:
                    loss = model(inputs_src=inputs_src, inputs_tgt=batch[1].to(args.device), labels_src=labels_src,
                                 attention_mask_src=attention_mask_src, attention_mask_tgt=attention_mask_tgt)
                    # print(loss)
                    tr_loss = backward_loss(loss, tr_loss)
                else:
                    loss = model(inputs_src=inputs_tgt, inputs_tgt=batch[0].to(args.device), labels_src=labels_tgt,
                                 attention_mask_src=attention_mask_tgt, attention_mask_tgt=attention_mask_src)
                    tr_loss = backward_loss(loss, tr_loss)
