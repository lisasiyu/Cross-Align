...
#for ln_pair in "roen" "deen" "enfr" "chen"; do
for ln_pair in "jaen";do
  ...
  if (( $# == 1 )); then
    ...
    ../scripts/bpe_to_word_alignments.py --src ${test_src} --tgt ${test_tgt} --input ${ln_pair}${bpe}.test.talp --output ${hypo}
    ../scripts/bpe_to_word_alignments.py --src ${test_tgt} --tgt ${test_src} --input ${ln_pair}${bpe}.test.reverse.talp --output ${hypo_reverse}
