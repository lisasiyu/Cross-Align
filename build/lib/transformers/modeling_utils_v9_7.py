...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]], *model_args, **kwargs):
        ...
        #cross bert key
        self_m = kwargs.pop("self_m", None)
        cross_k = kwargs.pop("cross_k", None)
    ...
    def _load_state_dict_into_model(
        cls, model, state_dict, pretrained_model_name_or_path, self_m, cross_k, ignore_mismatched_sizes=False, _fast_init=True,
    ):
        copy_state_dict = state_dict.copy()
        for key in state_dict.keys():
            new_key = None
            new_key1 = None
            # update new keys for cross bert
            if cross_k or self_m:
                import re
                res = re.search('\d+', key)
                # new_key = key.replace("bert.","crossbert.")
                new_key = key
                new_key1 = key


                if res:
                    layer_idx = int(res.group())
                    if layer_idx < self_m:
                        new_key = new_key.replace("bert.encoder.layer.{}".format(layer_idx), "bert.domain1.{}".format(layer_idx))
                        if "gamma" in new_key:
                            new_key = new_key.replace("gamma", "weight")
                        if "beta" in new_key:
                            new_key = new_key.replace("beta", "bias")
                        copy_state_dict[new_key] = state_dict[key]
                    elif layer_idx >= self_m and layer_idx < self_m + cross_k:
                        new_key = new_key.replace("attention", "crossattention")
                        new_key = new_key.replace("bert.encoder.layer.{}".format(layer_idx),
                                               "bert.inter.{}".format(layer_idx-self_m))


                        new_key1 = new_key1.replace("bert.encoder.layer.{}".format(layer_idx),
                                               "bert.intra.{}".format(layer_idx-self_m))
                        ...
                        if "beta" in new_key:
                            new_key = new_key.replace("beta", "bias")
                        copy_state_dict[new_key] = state_dict[key]
                        if "gamma" in new_key1:
                            new_key1 = new_key1.replace("gamma", "weight")
                        if "beta" in new_key1:
                            new_key1 = new_key1.replace("beta", "bias")
                        copy_state_dict[new_key1] = state_dict[key]

                    else:
                        new_key = new_key.replace("bert.encoder.layer.{}".format(layer_idx),
                                                  "bert.domain2.{}".format(layer_idx - self_m - cross_k))
                ...
                if new_key:
                    ...
                    copy_state_dict[key] = state_dict[key]

        state_dict = copy_state_dict
