...
def train(args, train_dataset, model: PreTrainedModel, tokenizer: PreTrainedTokenizer) -> Tuple[int, float]:
    ...
    def collate(examples):
        ...
        for example in examples:
            ...
            pos_src_id = example[-2][:args.block_size]
            pos_tgt_id = example[-1][:args.block_size]

            if random.random() > 0.5:
                # (pos,pos) pair
                psi_examples_src.append(pos_src_id)
                psi_examples_tgt.append(pos_tgt_id)
                psi_labels.append(1)
            else:
                # (pos,neg) pair
                if random.random() > 0.5:
                    psi_examples_src.append(src_id)
                    psi_examples_tgt.append(pos_tgt_id)
                else:
                    psi_examples_src.append(pos_src_id)
                    psi_examples_tgt.append(tgt_id)
                psi_labels.append(0)
        ...
        psi_examples_src = pad_sequence(psi_examples_src, batch_first=True, padding_value=tokenizer.pad_token_id)
        psi_examples_tgt = pad_sequence(psi_examples_tgt, batch_first=True, padding_value=tokenizer.pad_token_id)
        ...
        if args.n_gpu > 1 or args.local_rank != -1:
            ...
            guides = model.get_aligned_word(examples_src, examples_tgt, bpe2word_map_src, bpe2word_map_tgt, args.device,
                                            src_len, tgt_len, extraction=args.extraction,
                                            softmax_threshold=args.softmax_threshold, word_aligns=word_aligns)

        return examples_src, examples_tgt, guides, psi_examples_src, psi_examples_tgt, psi_labels
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.train_tlm:
                ...
                loss_inter = model(inputs_src=inputs_tgt, inputs_tgt=inputs_src,
                                               align_layer=args.align_layer, labels_src=labels_tgt,
                                               labels_tgt=labels_src, attention_mask_src=attention_mask_tgt,
                                               attention_mask_tgt=attention_mask_src, train_tlm=args.train_tlm
                                               )
                loss = loss_inter
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    print("  Step {}. Training loss = {}. Inter loss = {}.".format(str(global_step),
                                str((tr_loss - logging_loss) / args.logging_steps),str(loss_inter.mean().item())))
